...
function fileExistsRoute(urlPath){
  return routeCandidates(urlPath).some(c=>fs.existsSync(path.join(ROOT,c)));
}
...
for(const r of refs){
  const h=r.href;
  if(!h || h.startsWith('data:') || h.startsWith('javascript:')) continue;
  if(h.startsWith('#')){
    const id=h.slice(1);
  ...
  if(u.protocol==='http:' || u.protocol==='https:'){
    if(u.hostname===SITE_HOST || u.hostname===`www.${SITE_HOST}`){
      ...
      external.set(u.href,{url:u.href,origins:[...(external.get(u.href)?.origins||[]),r.source]});
    }
...
async function testExternal(item){
  ...
  try{
    let res=await fetch(item.url,{method:'HEAD',redirect:'follow',signal:controller.signal,headers:{'user-agent':'Mozilla/5.0 link-audit/1.0'}});
    if([400,403,405].includes(res.status)){
      res=await fetch(item.url,{method:'GET',redirect:'follow',signal:controller.signal,headers:{'user-agent':'Mozilla/5.0 link-audit/1.0'}});
    }
    const s=res.status;
    let status='PASS';
...
for(let i=0;i<extItems.length;i+=6){
  extResults.push(...await Promise.all(extItems.slice(i,i+6).map(testExternal)));
}
...
for(const r of results.sort((a,b)=>a.status.localeCompare(b.status)||a.source.localeCompare(b.source))){
  if(r.status!=='PASS') console.log(`${r.status}\t${r.source}\t${r.href}\t${r.detail}`);
}

console.log('\n## EXTERNAL RESULTS');
for(const r of extResults.sort((a,b)=>a.status.localeCompare(b.status)||a.url.localeCompare(b.url))){
  console.log(`${r.status}\t${r.http ?? '-'}\t${r.url}\t${r.finalUrl || r.error || ''}`);
}

const brokenInternal=results.filter(r=>r.status==='BROKEN');
const termA='route';
const termB='ly';
const forbidden=new RegExp(termA+termB,'i');
const forbiddenHits=[];
for(const f of files){
  const t=fs.readFileSync(f,'utf8');
  if(forbidden.test(t)) forbiddenHits.push(rel(f));
}
if(forbiddenHits.length){
  console.log(`\nFORBIDDEN BRAND REFERENCES: ${forbiddenHits.join(', ')}`);
}

if(brokenInternal.length || forbiddenHits.length){
  process.exitCode=1;
}
